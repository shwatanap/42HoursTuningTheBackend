...
const allActive = async (req, res) => {
  ...
  for (let i = 0; i < recordResult.length; i++) {
    ...
    if (results[0][0].length === 1) {
      createdByName = results[0][0][0].name;
    }
    if (results[1][0].length === 1) {
      applicationGroupName = results[1][0][0].name;
    }
    if (results[2][0].length === 1) {
      thumbNailItemId = results[2][0][0].item_id;
    }
    if (results[3][0].length === 1) {
      commentCount = results[3][0][0]['count(*)'];
    }
    if (results[4][0].length === 1) {
      mylog(updatedAt);
      const updatedAtNum = Date.parse(updatedAt);
